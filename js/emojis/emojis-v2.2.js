...
(async () => {
    try {
        ...
        const NONqualifiedEmojisArray = allemojiJSON.emojis.filter(e => e.status === "minimally-qualified" || e.status === "unqualified")
        const NONqualifiedEmojis = NONqualifiedEmojisArray.map(e => e.preview);
        ...
        function handleOtherInputs(x){

            const containsElement = elem => x.includes(elem);
            const foundElem = NONqualifiedEmojis.find(containsElement);
            if (foundElem) {
                loading.innerHTML = ''
                errorAlert.innerHTML = `Seems like your input "${x}" contains unqualified emoji "${foundElem}" (which rendered as "<span class='emojitxt'>${foundElem}</span>" in Input Field)`
                emojiList.innerHTML = `<p class='mytxt'>Please use only fully-qualified emojis as Unicode 15.0. <br> For More Details <a href='https://unicode.org/Public/emoji/15.0/emoji-test.txt' target="_blank">Click Here<a><br>Type "allGoogle" and copy the latest qualified emoji you want <br>Also try to update your keyboard</p>`
            } else {
                loading.innerHTML = ''
        ...
        function getinputData() {
            ...
            loading.innerHTML = `<i class="fa-solid fa-spinner fa-spin-pulse h1-title"></i>`
                ...
                else if (x === 'allgooglepng' || x==='allpng') {
                        getAllGoogleEmojisPngSvg('png')
                }
                else if (x === 'allgooglesvg' || x==='allsvg') {
                        getAllGoogleEmojisPngSvg('svg')
                }
                else if (x === 'allgoogle' || x==='allgoogletxt') {  
                        getAllGoogleEmojistxt()
                }
                else if (x === 'alltxt' || x==='alltxt') {  
                ...
                else if (x === 'allfacebook' || x==='allfb') {
                        getAllEmojis('facebook')
                }
                else if (x === 'allyoutube' || x==='allyt') {
                        getAllYouTubeEmojis()
                }
                else if (x === 'allanimated' || x==='allgif' || x==='allwebp'){   
                        getAllNotoAnimatedEmojis()
                }
                else if (x.toLowerCase() === 'ayexzilla'){   
                    loading.innerHTML = ''
                    emojiList.innerHTML = `<p class='bigtxt'>🪄 🦅</p>`
                }
        ...
        submitBtn.addEventListener("click",getinputData),input.addEventListener("keypress",(function(t){"Enter"===t.key&&(t.preventDefault(),submitBtn.click())}));    
    } 
    catch (error) {
...
//separated v2.2 on gh
// added local previews add manually qualified unicodes